...
class AdminCenterResource extends JsonResource
{
    public function toArray($request)
    {
        return [
            ...
            'admin' => $this->centerAdmin,
            'enquiry_sumary'=> $this->getTotalConversationsByType(),
            'applications_summary' => $this->getApplicationsSummary(),
    ...
    public function organizeFeatures($features)
{
    ...
    protected function getTotalConversationsByType()
    {
        $conversations = $this->conversations;
        
        $totalEnquiry = $conversations->where('type', 'enquiry')->count();
        $totalInformation = $conversations->where('type', 'information')->count();
        $totalBookTour = $conversations->where('type', 'book_tour')->count();
        $totalVacancies = $conversations->where('type', 'vacancies')->count();

        return [
            'enquiry' => $totalEnquiry,
            'information' => $totalInformation,
            'book_tour' => $totalBookTour,
            'vacancies' => $totalVacancies,
        ];
    }  


    protected function getApplicationsSummary()
    {
        $applications = $this->applications;

        $totalApplications = $applications->count();
        $pendingApplications = $applications->whereIn('status', ['pending', 'submitted'])->count();
        $approvedApplications = $applications->where('status', 'completed')->count();
        $waitingApplications = $applications->where('status', 'waitlisted')->count();
